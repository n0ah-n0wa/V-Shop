...
- Business rules (stamp threshold, prize weights, 5th-purchase spin, referral payout) belong to callers; these services take amounts as arguments.
- SQLite ignores `FOR UPDATE`. Concurrency is only proven by the opt-in `tests/test_loyalty_postgres.py` (`VSHOP_TEST_POSTGRES_URL`, database name must end in `_test` and be empty — it creates and drops the schema).
